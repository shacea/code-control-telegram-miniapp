...
fn debug_log(msg: &str) {
    ...
    let enabled = ENABLED.get_or_init(|| {
        std::env::var("CODE_CONTROL_TELEGRAM_DEBUG")
            .or_else(|_| std::env::var("CODE_CONTROL_TELEGRAM_DEBUG"))
            .or_else(|_| {
                let val = std::env::var("COKACDIR_DEBUG");
                if val.is_ok() {
                    eprintln!("[DEPRECATED] legacy DEBUG env var is deprecated; use CODE_CONTROL_TELEGRAM_DEBUG");
                }
                val
            })
            .map(|v| v == "1")
    ...
    if let Some(home) = dirs::home_dir() {
        let debug_dir = home.join(".code-control-telegram").join("debug");
...
/// Get the AI sessions directory path (~/.code-control-telegram/ai_sessions)
pub fn ai_sessions_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".code-control-telegram").join("ai_sessions"))
}
...
impl AIScreenState {
    ...
    /// Save current session to file (~/.code-control-telegram/ai_sessions/[session_id].json)
